...
CREATE SEQUENCE UM_ORG_USER_ASSOC_SEQUENCE AS DECIMAL(27,0)
    INCREMENT BY 1
    START WITH 1
    NO CACHE/

CREATE TRIGGER UM_ORG_USER_ASSOC_TRIG NO CASCADE BEFORE INSERT ON UM_ORG_USER_ASSOCIATION
REFERENCING NEW AS NEW FOR EACH ROW MODE DB2SQL
BEGIN ATOMIC
    SET (NEW.UM_ID)
       = (NEXTVAL FOR UM_ORG_USER_ASSOC_SEQUENCE);END/
